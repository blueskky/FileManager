...
package com.whty.eschoolbag.filemanager.db;
...
public class FavoriteDatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean insert(String title, String path) {
        if (isFavorite(path))
            return false;

        SQLiteDatabase db = this.getWritableDatabase();
        long ret = db.insert(TABLE_NAME, null, createValues(title, path));
        if (mListener != null) {
    ...
    public boolean delete(long id, boolean notify) {
        SQLiteDatabase db = this.getWritableDatabase();
        String where = FIELD_ID + "=?";
        String[] whereValue = {
                Long.toString(id)
        };
        int delete = db.delete(TABLE_NAME, where, whereValue);
